...
-- flag controlling whether we enforce an API key.  The logic is preserved
-- but the check is turned off for now so the hub runs without requiring one.
local keySystemEnabled = false -- set to `true` when the API key mechanism is ready

if keySystemEnabled then
    -- initial default key value; will be overwritten if `script_key` exists
    _G.MY_KEY_IS = "No_Key"
    if script_key then
    ...
    -- API key system disabled: always use a dummy key to avoid errors
    _G.MY_KEY_IS = "No_Key"
end
-- if _G.HohoVersion == "v3" or _G["Fumo Blox Fruit - LUMINNOUS Hub Kaitun V3"] or _G["LUMINNOUS Hub Auto Bounty V3"] or _G["HoHo Fruits Collector"] or _G.HOHO_PVP_UI or _G.HOHO_SERVER_ID then
...
--         Label.Text = "LUMINNOUS ADS (contact discrod.gg/luminoushub) (auto close when 60s)"
--         Label.Parent = Back
...
--     -- GroupADS:CreateAds(GetImage("hohodiscord", "https://i.imgur.com/e2fdiZD.png"), "https://discord.gg/luminoushub")
-- end)

-- note: notification suppressed when key system disabled (see keySystemEnabled)
...
if list[game.GameId] ~= nil then
	...
	notify.New("This game is not supported by LUMINNOUS Hub :(", 60)
	notify.New("Please join discord.gg/luminoushub to check what game r supported by us!", 60)
	notify.New("Running LUMINNOUS Universal Script!", 60)
...
-- if _G["LUMINNOUS Hub Auto Bounty V3"] or _G["Fumo Blox Fruit - LUMINNOUS Hub Kaitun V3"] then return end
