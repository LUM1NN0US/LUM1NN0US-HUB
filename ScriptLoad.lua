...
-- load external key system (if present).  The helper file should return a
-- table with `copyLink`, `verifyKey` and `getFlag` functions.  Messages are
-- routed through StarterGui so the user sees status updates.
local copyLink, verifyKey, getFlag
if isfile("KeySystem.lua") then
	local ok, ks = pcall(loadstring(readfile("KeySystem.lua")))
	if ok and type(ks) == "table" then
		copyLink = ks.copyLink
		verifyKey = ks.verifyKey
		getFlag = ks.getFlag
		ks.onMessage = ks.onMessage or function(msg)
...
do	
	LUMINNOUS_Passcheck = Instance.new("ScreenGui")
	...
	LUMINNOUS_Passcheck.IgnoreGuiInset = true
	LUMINNOUS_Passcheck.ResetOnSpawn = false
	LUMINNOUS_Passcheck.Name = "LUMINNOUS_passcheck"
	HOHO_Passcheck.ScreenInsets = Enum.ScreenInsets.DeviceSafeInsets
	HOHO_Passcheck.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    CoreGuiAdd(LUMINNOUS_Passcheck)
	LUMINNOUS_Passcheck.Enabled = true
	...
	INTRO.Parent = LUMINNOUS_Passcheck
	...
	GET_KEY.Parent = LUMINNOUS_Passcheck
	...
	LUMINNOUS_Gen4 = Instance.new("ScreenGui")
	...
	LUMINNOUS_Gen4.IgnoreGuiInset = true
	LUMINNOUS_Gen4.Enabled = true
	LUMINNOUS_Gen4.ResetOnSpawn = false
	LUMINNOUS_Gen4.Name = "LUMINNOUS_gen4"
	HOHO_Gen4.ScreenInsets = Enum.ScreenInsets.DeviceSafeInsets
	HOHO_Gen4.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    CoreGuiAdd(LUMINNOUS_Gen4)
	...
	NOTIFICATION_ZONE.Parent = LUMINNOUS_Gen4
    ...
    if (isfile("LUMINNOUS_Intro.txt") and (tick() - tonumber(readfile("LUMINNOUS_Intro.txt"))) >= 86400) or not isfile("LUMINNOUS_Intro.txt") then
        writefile("LUMINNOUS_Intro.txt", tostring(tick())) 
        
        local preload_content = {}
        for i,v in pairs(LUMINNOUS_Passcheck:GetDescendants()) do
	...
	local destroyUI = function()
		LUMINNOUS_Passcheck:Destroy()
		LUMINNOUS_Gen4:Destroy()
	end
	...
	-- override key checking if external system provides verifyKey
	if verifyKey then
		local old_check = do_check_key
		do_check_key = function(key)
			if checking_key then return end
			checking_key = true
			-- try external verification first
	...
	Get.MouseButton1Click:Connect(function()
		-- setclipboard('https://luminouscommunity.xyz/api/getkeyv4?hwid=' .. tick())
	...
	do
		if isfile("LUMINNOUSKeyV4.txt") or _G.MY_KEY_IS then
			local key = _G.MY_KEY_IS and _G.MY_KEY_IS or readfile("LUMINNOUSKeyV4.txt")
